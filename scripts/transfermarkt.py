...
def _parse_mv_to_millions(mv):
    """Zamienia wartość rynkową (np. '€3.50m', '€800k', liczba) na mln EUR (float)."""
    if mv is None:
        return 0.0
    if isinstance(mv, (int, float)):
        return round(float(mv) / 1_000_000, 2) if mv > 10000 else float(mv)
    s = str(mv).strip().lower().replace("€", "").replace("\u20ac", "").replace(",", "")
    try:
        if s.endswith("m"):
